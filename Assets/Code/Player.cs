...
public class Player : MonoBehaviour
{
	...
	private IEnumerator Dash()
	{
		...
		while (progress < 1f)
		{
			...
			Phone.Instance.DashImage.fillAmount = 1 - progress;
	...
	private IEnumerator DashCooldown(float coolDownTime = 10f)
	{
		...
		while (coolDownTime > 0.0f)
		{
			...
			// Update visual
			Phone.Instance.DashImage.fillAmount = progress;
	...
	private void UpdateProgress(bool reset = false)
	{
		...
		Phone.Instance.DownloadPercentageText.text = string.Format("Downloading: {0}%", (int)(progress * 100));
	}
